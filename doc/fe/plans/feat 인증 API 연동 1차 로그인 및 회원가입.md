...
개발 서버는 `https://gone-dev.gbsw.hs.kr`이며, 실제 인증 API는 `/api/v1/auth` prefix를 사용합니다. 로그인·회원가입·전화번호 인증 계약은 백엔드 저장소의 인증 API 문서와 개발 서버 응답으로 확인했습니다.
...
- Notion 인증 상세 페이지는 비어 있었고, 백엔드 저장소의 인증 API 문서를 기준으로 전화번호 인증 계약을 보완했습니다.
- 서버 회원가입 요청은 `loginId`, `password`, `phoneNumber`, `ticket`만 사용하며, 현재 화면의 학번·이름·프로필 이미지는 후속 사용자 API 범위입니다.
- Moya가 프로젝트에 등록되어 있지 않아 패키지 추가 및 네트워크 환경 접근이 필요합니다.
- 토큰 응답 구조가 확인되기 전에는 자동 로그인·토큰 갱신을 구현하지 않습니다.

## 7. 완료 기준

- [x] 승인된 API 계약으로 로그인·회원가입 요청이 실제 서버에 전달된다.
- [x] 전화번호 인증번호 발송·검증 후 발급된 `ticket`을 회원가입 요청에 전달한다.
